"""Generate SVG comparison plots for the per-class GMM cluster
features experiment.

Reads ``results/cluster_per_class.csv`` (produced by
``src.bench_cluster``) and writes:

* ``v2_cluster_macro_f1.svg``  — macro F1 on test + genericity for the
                                  five v2 models, A+B vs A+B+cluster.
* ``v2_cluster_per_class.svg`` — per-class F1 (Suspicious + DDoS),
                                  test + genericity, A+B vs A+B+cluster.
"""
...
COLOR_AB = "#10b981"          # emerald — Fix A+B (baseline for this comparison)
COLOR_AB_CLUSTER = "#f59e0b"  # amber — A+B + cluster features

CONFIG_ORDER = ["ab", "ab_cluster"]
CONFIG_LABEL = {
    "ab":         "A+B (21 feat)",
    "ab_cluster": "A+B + per-class GMM (36 feat)",
}
CONFIG_COLOR = {"ab": COLOR_AB, "ab_cluster": COLOR_AB_CLUSTER}
...
def plot_macro(df: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        "Per-class GMM cluster features — macro F1 effect",
        fontsize=14, fontweight="bold", y=1.02,
    )
    fig.tight_layout()
    path = PLOTS_DIR / "v2_cluster_macro_f1.svg"
...
def plot_per_class(df: pd.DataFrame) -> None:
    ...
    for ax, ds, cls, title in cells:
        for i, cfg in enumerate(CONFIG_ORDER):
            ...
            bars = ax.bar(x + offset, vals, width, label=CONFIG_LABEL[cfg],
                          color=CONFIG_COLOR[cfg], edgecolor="white",
                          linewidth=0.5)
            _annotate(ax, bars, fs=6.5, fmt="{:.2f}")
    ...
    fig.suptitle(
        ...
        "A+B vs A+B + per-class GMM features",
        fontsize=13, fontweight="bold", y=1.0,
    )
    fig.tight_layout()
    path = PLOTS_DIR / "v2_cluster_per_class.svg"
    fig.savefig(path, format="svg", bbox_inches="tight")
    plt.close(fig)
    print(f"  saved {path.name}")
...
def main() -> None:
    ...
    df = pd.read_csv(RESULTS_DIR / "cluster_per_class.csv")
    print("Generating cluster-feature comparison plots...")
    plot_macro(df)
    plot_per_class(df)
    print("Done.")
